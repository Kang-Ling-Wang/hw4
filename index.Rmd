...
```{r}
# Write your code here
len_below25 <- df$len < 25
df[vec_supp_VC & len_below25, ]
# should print out:
#> # A tibble: 24 x 3
#>      len supp   dose
#>    <dbl> <fct> <dbl>
...
```{r}
# Write your code here
df_leng_plus15_times_dose_biggerthan40 <- (df$len + 15)*df$dose > 40
df[df_leng_plus15_times_dose_biggerthan40, ]
# should print out:
#> # A tibble: 24 x 3
#>      len supp   dose
#>    <dbl> <fct> <dbl>
...
1. 修改 list 裡面元素的語法與 vector 類似：
    ...
    lst <- list(
           ...
           list(name = "tiger", age = 19),
           list(name = "piglet", age = 18))
...
for (i in seq_along(member)) {
  n_years_later <- function(member, y) {
   m1 <- member[[1]]
   m2 <- member[[2]]
   m3 <- member[[3]]
   m4 <- member[[4]]
}
   phrases <- c(m1$age - y, m2$age - y, m3$age - y, m4$age - y)
}
   print(phrases)
};
   n_years_later <- function(member, y) {
...
# Write your code here
library(readr)
cute_cute <- read_csv(".\\week3Rclass.csv")
...
```{r}
# Write your code here
cute_cute$favorite_word = "a"
cute_cute["favorite_word"][cute_cute["q_self"]>=70] <- "根本可愛動物區"
